...
export function AgentsConfig({
  ...
  const loadProfiles = useCallback(async (preferredKey?: string) => {
    ...
    setError(null);
    try {
      const response = await window.pi.subagentsProfilesList(cwd);
      const data = response.body as Partial<SubagentProfilesResponse> & { error?: string };
      if (response.status !== 200 || data.error) throw new Error(data.error ?? `HTTP ${response.status}`);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await window.pi.subagentsSettingsGet();
        if (controller.signal.aborted) return;
        const data = response.body as Partial<SubagentSettingsResponse> & { error?: string };
        if (response.status !== 200 || data.error || typeof data.enabled !== "boolean") {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await window.pi.models(cwd);
        if (controller.signal.aborted) return;
        const data = response.body as Partial<ModelsData> & { error?: string };
        if (response.status !== 200 || data.error) throw new Error(data.error ?? `HTTP ${response.status}`);
  ...
  const save = async () => {
    ...
    setSavedOk(false);
    try {
      const response = await window.pi.subagentsProfilesSave({ cwd, scope: targetScope, profile: draft });
      const data = response.body as { profile?: SubagentProfile; error?: string };
      if (response.status !== 200 || data.error || !data.profile) throw new Error(data.error ?? `HTTP ${response.status}`);
  ...
  const remove = async () => {
    ...
    setError(null);
    try {
      const response = await window.pi.subagentsProfilesDelete({ cwd, scope: selected.scope, name: selected.name });
  ...
  const toggleEnabled = async (enabled: boolean) => {
    ...
    setError(null);
    try {
      const response = await window.pi.subagentsProfilesToggle({ cwd, scope: selected.scope, name: selected.name, enabled });
      const data = response.body as { profile?: SubagentProfile; error?: string };
      if (response.status !== 200 || data.error || !data.profile) throw new Error(data.error ?? `HTTP ${response.status}`);
  ...
  const toggleBuiltInSubagents = async (enabled: boolean) => {
    ...
    setSettingsError(null);
    try {
      const response = await window.pi.subagentsSettingsPut(enabled);
      const data = response.body as Partial<SubagentSettingsResponse> & { error?: string };
      if (response.status !== 200 || data.error || typeof data.enabled !== "boolean") {
        throw new Error(data.error ?? `HTTP ${response.status}`);
      }
  ...
  const updateMaxConcurrent = async (value: number) => {
    ...
    setSettingsError(null);
    try {
      const response = await window.pi.subagentsSettingsPut(undefined, value);
      const data = (response.body ?? {}) as Partial<SubagentSettingsResponse> & { error?: string };
      if (response.status !== 200 || data.error || typeof data.maxConcurrent !== "number") throw new Error(data.error ?? `HTTP ${response.status}`);
