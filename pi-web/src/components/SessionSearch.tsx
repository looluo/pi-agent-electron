...
export function SessionSearch({ open, query, refreshKey, children, selectedSessionId, onSelectSession }: {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        // Typed IPC instead of the upstream HTTP route; AbortSignal cannot cross
        // the bridge, so stale results are dropped via the controller flag.
        const res = await window.pi.sessionsSearch(search);
        if (res.status !== 200 || !res.body) throw new Error(`search failed (${res.status})`);
        const data = res.body as SessionSearchResponse;
