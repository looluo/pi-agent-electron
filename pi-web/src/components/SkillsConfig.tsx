...
function AddSkillPanel({
  ...
  const search = useCallback(async (q: string) => {
    ...
    setResults([]);
    try {
      const result = await window.pi.skillsSearch(q.trim(), undefined);
      const d = (result.body ?? {}) as unknown as {
  ...
  const install = useCallback(
    async (pkg: string) => {
      ...
      try {
        const result = await window.pi.skillsInstall({ package: pkg, scope, cwd });
...
export function SkillsConfig({
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      ...
      const d = (result.body ?? {}) as unknown as Partial<SkillsResponse> & { error?: string };
  ...
  const checkForUpdates = useCallback(async (skill?: Skill) => {
    ...
    try {
      const result = await window.pi.skillsCheck({
        cwd,
      ...
      if (result.status !== 200 || data.error) throw new Error(data.error ?? "Update check failed");
  ...
  const updateInstalledSkill = useCallback(async (skill: Skill) => {
    ...
    setUpdateError(null);
    try {
      const result = await window.pi.skillsUpdate({
        cwd,
        package: skill.install.package,
        scope: skill.install.scope,
      });
      ...
      if (result.status !== 200 || data.error || !data.success) {
