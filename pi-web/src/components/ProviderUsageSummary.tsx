...
function ProviderUsageContent({ providerId, enabled }: { providerId: string; enabled: boolean }) {
  ...
  const query = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await window.pi.providerUsageQuery(providerId) as UsageResponse & { error?: string };
      if (result === null || result === undefined) throw new Error(t("providerUsage.queryFailed"));
