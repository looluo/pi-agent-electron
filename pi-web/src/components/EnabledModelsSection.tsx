...
export function useEnabledModels(cwd?: string | null): EnabledModelsController {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    window.pi.modelsEnabledGet(cwd ?? null)
      .then((raw) => {
        const res = raw as { status: number; body: Record<string, unknown> | null };
        const data = (res.body ?? {}) as unknown as EnabledModelsView & { error?: string };
  ...
  const mutate = useCallback((key: string, body: MutationBody) => {
    ...
    void (async () => {
      try {
        const raw = await window.pi.modelsEnabledPut({ ...body, ...(cwd ? { cwd } : {}) });
        const res = raw as { status: number; body: Record<string, unknown> | null };
        const data = (res.body ?? {}) as unknown as EnabledModelsView & { error?: string; reason?: string };
...
function useCatalogRefresh(providerId: string, onChanged: () => void) {
  ...
  const refresh = useCallback(() => {
    ...
    void (async () => {
      try {
        const raw = await window.pi.modelsCatalogRefresh(providerId);
        const res = raw as { status: number; body: Record<string, unknown> | null };
        const data = (res.body ?? {}) as unknown as CatalogRefreshResponse;
        if (res.status !== 200 || data.error) throw new Error(data.error ?? `HTTP ${res.status}`);
