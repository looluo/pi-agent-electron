...
export function PluginsConfig({
  ...
  const loadPlugins = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await window.pi.pluginsList(cwd);
      const next = (result.body ?? {}) as unknown as PluginsResponse & { error?: string };
      if (result.status !== 200 || next.error) throw new Error(next.error ?? "Failed to load plugins");
      setData(next);
  ...
  const checkForUpdates = useCallback(async (pkg?: PluginPackageInfo) => {
    ...
    try {
      const res = await window.pi.pluginsCheck({
        cwd,
        source: pkg?.source,
        scope: pkg?.scope,
      });
      ...
      if (res.status !== 200 || data.error) throw new Error(data.error ?? `HTTP ${res.status}`);
  ...
  const updateAllPluginsAction = useCallback(async () => {
    ...
    setUpdateError(null);
    try {
      const result = await window.pi.pluginsAction({ action: "update", cwd });
      const next = (result.body ?? {}) as unknown as PluginsResponse & { error?: string };
      if (result.status !== 200 || next.error) throw new Error(next.error ?? "Update failed");
      setData(next);
      setUpdateStatuses({});
  ...
  const runAction = useCallback(async (action: PluginAction, pkg: PluginPackageInfo) => {
    ...
    setActionMessage(null);
    try {
      const result = await window.pi.pluginsAction({ action, source: pkg.source, scope: pkg.scope, cwd });
      const next = (result.body ?? {}) as unknown as PluginsResponse & { error?: string };
      if (result.status !== 200 || next.error) throw new Error(next.error ?? "Plugin action failed");
      setData(next);
      if (action === "remove") {
  ...
  const installPlugin = useCallback(async () => {
    ...
    setActionMessage(null);
    try {
      const result = await window.pi.pluginsAction({ action: "install", source, scope: installScope, cwd });
      const next = (result.body ?? {}) as unknown as PluginsResponse & { error?: string };
      if (result.status !== 200 || next.error) throw new Error(next.error ?? "Install failed");
      setData(next);
