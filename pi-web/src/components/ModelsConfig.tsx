...
function OAuthDetail({ provider, onRefresh }: { provider: OAuthProvider; onRefresh: () => void }) {
  ...
  const submitSelection = useCallback(async (token: string, value: string) => {
    ...
    try {
      const result = await window.pi.authLoginCode(provider.id, token, value);
      if (result.status !== 200) {
        const d = (result.body ?? {}) as { error?: string };
        setLoginState({ phase: "error", message: d.error ?? "Login failed" });
      }
...
export function ModelsConfig({ onClose, embedded = false }: { onClose: () => void; embedded?: boolean }) {
  ...
  const refreshAuthProviders = useCallback(() => {
    window.pi.authProviders()
      .then((raw) => raw as unknown as { oauthProviders?: OAuthProvider[]; apiKeyProviders?: ApiKeyProvider[] })
      .then((d) => {
