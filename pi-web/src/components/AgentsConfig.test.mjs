...
test("offers a persisted built-in sub-agent switch with explicit session reload", () => {
  assert.match(source, /window\.pi\.subagentsSettingsPut\(enabled\)/);
  assert.match(source, /window\.pi\.subagentsSettingsGet\(\)/);
...
test("sends the selected scope for saves and the source scope for deletes", () => {
  assert.match(source, /window\.pi\.subagentsProfilesSave\(\{ cwd, scope: targetScope, profile: draft \}\)/);
  assert.match(source, /window\.pi\.subagentsProfilesDelete\(\{ cwd, scope: selected\.scope, name: selected\.name \}\)/);
});
...
test("persists existing profile toggles immediately without submitting unsaved fields", () => {
  ...
  assert.match(source, /window\.pi\.subagentsProfilesToggle\(/);
  assert.match(source, /window\.pi\.subagentsProfilesToggle\(\{ cwd, scope: selected\.scope, name: selected\.name, enabled \}\)/);
  assert.match(source, /setDraft\(\(current\) => \(\{ \.\.\.current, enabled: saved\.enabled \}\)\)/);
  assert.doesNotMatch(source, /subagentsProfilesToggle\([\s\S]*?profile: draft/);
});

test("reuses the ChatInput model selector with scoped models", () => {
  assert.match(source, /window\.pi\.models\(cwd\)/);
...
test("confirms deletion and limits it to writable profiles", () => {
  ...
  assert.match(source, /selected && isWritableScope\(selected\.scope\) && mode === "edit"/);
  assert.match(source, /window\.pi\.subagentsProfilesDelete\(/);
});
