...
test("keeps the session event stream open through the idle grace window", () => {
  ...
  assert.match(graceSource, /window\.pi\.agentState\(sid\)/);
...
test("stale fresh-session completion cannot replace the active composer", () => {
  ...
  assert.match(cwdChangeSource, /if \(currentProject !== newProject\) \{[\s\S]*?switchProjectFileTabs\(currentProject, newProject\)/);
  assert.match(
    appShellSource,
...
test("keeps the selected session warm while idle and renews its lease", () => {
  assert.doesNotMatch(source, /sessionRunning\?: boolean/);
  assert.match(
    source,
  ...
  assert.match(source, /window\.pi\.agentLease\(sid\)/);
  ...
  assert.doesNotMatch(chatWindowSource, /sessionRunning/);
