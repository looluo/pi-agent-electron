...
export function useAgentSession(opts: UseAgentSessionOptions) {
  const {
    session, newSessionCwd, newSessionDraftKey, onAgentEnd, onTitleGenerated, onAttentionNeeded, onSessionCreated, onSessionForked,
  ...
  const flight = (async (): Promise<unknown> => {
    ...
    try {
      ...
      const raw = await window.pi.sessionsGet(sid, {
        deferThinking: true,
        deferMedia: true,
        tree: "summary",
  ...
  const ensureNewSession = useCallback(async () => {
    ...
    const promise = (async () => {
      ...
      const result = newResult.data as unknown as {
        sessionId: string;
  ...
  // Keep the selected session warm even while its agent is idle. The IPC
  // event-stream lease is renewed separately below and expires if the window
  // disappears.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const renewLease = async () => {
      ...
      renewing = true;
      try {
        const result = await window.pi.agentLease(sid) as { renewed?: number };
        if (disposed || result === null || result === undefined) return;
        if (
          !disposed
          && result.renewed === 0
          && sessionIdRef.current === sid
        ...
        // Retry on the next interval; the event connection remains the primary path.
      } finally {
        renewing = false;
      }
    };
  ...
  }, [addNotice, cancelEventStreamGrace, handleExtensionUiRequest, loadSession, maybeAutoNameSession, notifyPromptStage, onAgentEnd, scheduleEventStreamClose, scrollToBottom, settleUiStage, syncLiveModel]);
