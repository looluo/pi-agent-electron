...
test("all project resource loaders and reloads enforce project trust", async () => {
  ...
  const modelsSource = await readFile(new URL("../../../electron/main/services/models-auth.ts", import.meta.url), "utf8");
  const skillsSource = await readFile(new URL("./skills-service.ts", import.meta.url), "utf8");
  const skillsInstallSource = await readFile(new URL("../../../electron/main/services/skills.ts", import.meta.url), "utf8");
  const pluginsSource = await readFile(new URL("../../../electron/main/services/plugins.ts", import.meta.url), "utf8");
...
test("the trust API invalidates cached models and restricted runtimes", async () => {
  const source = await readFile(new URL("../../../electron/main/services/workspace.ts", import.meta.url), "utf8");
  const rpcSource = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
