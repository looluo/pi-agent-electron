  ...
  const content = message.content.flatMap((block, blockIndex) => {
    ...
    if (
      ...
      const source: ImageContent["source"] = {
        ...
        // Electron port: served by the pifile:// protocol's session host
        // (files-protocol.ts -> sessionToolResultImage) instead of an HTTP route.
        url: `pifile://session/${encodeURIComponent(sessionId)}/entries/${encodeURIComponent(entryId)}/tool-result-image?blockIndex=${blockIndex}`,
      };
