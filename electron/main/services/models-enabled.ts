...
/**
 * Port of app/api/models/enabled (GET + PUT) — per-provider model switches in
 * Settings → Models (upstream #930). Route-era error semantics: handlers
 * resolve { status, body } instead of rejecting.
 */
 ...
 * file, so it goes through the same allow-list as modelsGet.
 */
async function resolveCwd(raw: string | null): Promise<{ cwd: string } | StatusBody> {
...
export async function modelsEnabledGet(rawCwd: string | null): Promise<StatusBody> {
  const resolved = await resolveCwd(rawCwd);
  if ("status" in resolved) return resolved;
  try {
    return { status: 200, body: await buildView(await loadContext(resolved.cwd)) as unknown as Record<string, unknown> };
  } catch (error) {
...
export async function modelsEnabledPut(body: EnabledModelsRequest | undefined | null): Promise<StatusBody> {
  ...
  if (body.cwd !== undefined && typeof body.cwd !== "string") {
    return { status: 400, body: { error: "Invalid cwd" } };
  }
  ...
  try {
    ...
    if (scope === "project") {
      return {
        status: 409,
        body: { error: "Project settings override enabledModels", reason: "project-scope" },
      };
    }
    ...
    if (op === "clear") {
      ...
      if (!renames || !modelRenames || !fullyEnabled) {
        return { status: 400, body: { error: "Invalid resync payload" } };
      }
      ...
      if (op === "provider") {
        ...
        // the renderer has not seen yet.
        refs = context.models
          .filter((model) => model.provider === body.provider)
          .map(modelRef);
      } else {
        const requested = stringArray(body.refs);
        if (!requested) return { status: 400, body: { error: "refs must be an array of strings" } };
        refs = requested;
      }
    ...
    if (!edit.ok) {
      return {
        status: 409,
        body: { error: "At least one model must stay enabled", reason: edit.reason },
      };
    }
    if (edit.changed) {
    ...
    return { status: 200, body: await buildView(context) as unknown as Record<string, unknown> };
  } catch (error) {
    return { status: 500, body: { error: String(error) } };
  }
}
...
/**
 * Port of app/api/models/refresh (POST) — fetch the remote provider catalogs
 * now (upstream #914/#938). Body `{ provider }`; without one every
 * refreshable provider is passed over. The refreshed catalog itself reaches
 * the renderer through the modelsEnabledGet reload the caller does next.
 */
export async function modelsCatalogRefresh(provider: string | null): Promise<StatusBody> {
  const trimmed = provider?.trim();
  try {
    const result = await shareModelCatalogRefresh(trimmed ? { providers: [trimmed] } : {});
