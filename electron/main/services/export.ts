/**
 * Port of app/api/sessions/[id]/export — exports a session to patched HTML.
 * The route streamed the file; the service returns the temp path and the
 * renderer opens it with shell.openPath / a browser window.
 */
...
export async function sessionExport(id: string): Promise<{ status: number; body: Record<string, unknown> }> {
  try {
    ...
    return { status: 200, body: { path: outputPath, fileName } };
  } catch (error) {
